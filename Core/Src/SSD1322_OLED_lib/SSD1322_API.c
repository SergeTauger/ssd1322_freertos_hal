...
//		SSD1322_HW_SPI_send_byte(command);
		...
		*((uint8_t*)&(OLED_HSPI.Instance->DR)) = command;
		while(OLED_HSPI.Instance->SR & SPI_SR_BSY){
			;
		}
		OLED_HSPI.Instance->CR1 &= (~1<<SPI_CR1_SPE_Pos);
...
//		SSD1322_HW_SPI_send_byte(data);
		...
		/*
		OLED_HSPI.Instance->CR1 |= 1<<SPI_CR1_SPE_Pos;
		*((uint8_t*)&(OLED_HSPI.Instance->DR)) = data;
		while((OLED_HSPI.Instance->SR & SPI_SR_BSY)){
			;
		}
		OLED_HSPI.Instance->CR1 &= (~1<<SPI_CR1_SPE_Pos);
...
void SSD1322_API_deInit()
{
	initialized_flag = 0;
	SSD1322_API_command(0xAE);     //enter sleep mode, <10 uA
	//Tirn off internal reg with AB command? Seed schematics http://microsin.net/adminstuff/hardware/ssd1322-oled-controller.html
	SSD1322_API_command(0xFD);     //set Command lock
	SSD1322_API_data(0x16);
}
